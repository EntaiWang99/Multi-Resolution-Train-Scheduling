...
station_list = ['W7-B','W7-1','W7-II','W7-III','W7-4','W7-A']

# station_coord = [0,1,2,3,4,5,6,
#                  11,12,13,14,15,16,
#                  24,25,26,27,28,29,30]
station_coord = [11,12,13,14,15,16]
...
if __name__ == '__main__':
    ...
    # mpl.rcParams['font.sans-serif'] = ['SimHei'] # for chinese 
    # for english 
    legend_font = {"family" : "Times New Roman"}


    xlist = g_agent_list[2].time_list
    ylist = g_agent_list[2].node_list
    if(len(xlist) != 0):
        plt.plot(xlist, ylist, color = color_value[2], linewidth = 1.2, linestyle='--')
        plt.text(xlist[-1] - 80, ylist[-1], 'Train 2', ha='center', \
            va= 'bottom', color = color_value[2], weight = 'bold', fontsize= 20, family = 'Times new roman')

    xlist = g_agent_list[3].time_list
    ylist = g_agent_list[3].node_list
    if(len(xlist) != 0):
        plt.plot(xlist, ylist, color = color_value[3], linewidth = 1.2, linestyle='--')
        plt.text(xlist[-1] - 80, ylist[-1] + 0.1, 'Train 3', ha='center', \
            va= 'bottom', color = color_value[3], weight = 'bold', fontsize= 20, family = 'Times new roman')

    xlist = g_agent_list[4].time_list
    ylist = g_agent_list[4].node_list
    if(len(xlist) != 0):
        plt.plot(xlist, ylist, color = color_value[2], linewidth = 1.2)
        plt.text(xlist[-1] - 80, ylist[-1], 'Train 2', ha='center', \
            va= 'bottom', color = color_value[2], weight = 'bold', fontsize= 20, family = 'Times new roman')

    xlist = g_agent_list[5].time_list
    ylist = g_agent_list[5].node_list
    if(len(xlist) != 0):
        plt.plot(xlist, ylist, color = color_value[3], linewidth = 1.2)
        plt.text(xlist[-1] - 80, ylist[-1] + 0.1, 'Train 3', ha='center', \
            va= 'bottom', color = color_value[3], weight = 'bold', fontsize= 20, family = 'Times new roman')
    ...
    #     if(len(xlist) != 0):
    #         # if ((agent.agent_no == 2) or (agent.agent_no == 3)):
    ...
    plt.xlim(1200, 2700)
    plt.xticks([1200,1500,1800,2100,2400,2700], family = 'Times new roman', fontsize = 24)
 

    plt.yticks(station_coord, station_list, family = 'Times new roman', fontsize = 24)
    from pylab import mpl
    # mpl.rcParams['font.sans-serif'] = ['SimHei'] # for chinese only
    plt.xlabel('Time (min)', fontsize = 24, family = 'Times new roman')
    plt.ylabel('Station/Line track', fontsize = 24, family = 'Times new roman')
    plt.show()
    ...
    plt.xlabel('Itearion number', fontsize = 24)
    plt.ylabel('onflict number (train·station)', fontsize = 24)
