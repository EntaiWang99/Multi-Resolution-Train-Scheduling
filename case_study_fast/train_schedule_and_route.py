...
os.chdir("./H_1U1D_Fixed")

g_station_list = [6, 7, 8]
g_agent_map_dict = {}
g_agent_map = []
g_workbench_list = []
g_outer_iteration = 1
...
def g_ReadInputData(network_type):
    ...
    with open('road_link.csv', 'r') as fp:
        ...
        for l in lines[1:]:
            ...
            link.departure_dependent_travel_time_matrix = [
                0] * g_number_of_simulation_intervals
            link.arrival_dependent_travel_time_matrix = [
                0] * g_number_of_simulation_intervals

            g_node_list[link.from_node_seq_no].outgoing_link_list.append(
                link.link_seq_no)
    ...
    if (network_type == 'macro'):
        with open('input_agent.csv', 'r') as fp:
            ...
            index_earliest_departure_time = temp.index(
            ...
            for l in lines[1:]:
                ...
                try:
                    ...
                    agent.earliest_departure_time = int(
                        l[index_earliest_departure_time])
                    agent.latest_departure_time = int(
                        l[index_latest_departure_time])
                    ...
                    for k in range(0, len(allowed_links_list)):
                        if allowed_links_list[k] != '':
                            agent.set_of_allowed_links.append(
                                g_internal_link_seq_no_dict[int(allowed_links_list[k])])
                    ...
                    for k in range(0, len(min_link_travel_time_list)):
                        if min_link_travel_time_list[k] != '':
                            agent.min_link_travel_time.append(
                                int(min_link_travel_time_list[k]))
                    ...
                    for k in range(0, len(max_link_travel_time_list)):
                        if max_link_travel_time_list[k] != '':
                            agent.max_link_travel_time.append(
                                int(max_link_travel_time_list[k]))
...
def optimal_ST_dynamic_programming_LR(arg, earlest_departure_time, arrival_time, LR_iteration):
    ...
    m_label_cost = [[MAX_LABEL_COST for i in range(
        g_number_of_simulation_intervals)] for i in range(g_number_of_nodes)]
    m_node_predecessor = [
        [-1 for i in range(g_number_of_simulation_intervals)] for i in range(g_number_of_nodes)]
    m_time_predecessor = [
        [-1 for i in range(g_number_of_simulation_intervals)] for i in range(g_number_of_nodes)]
            ...
            for link in g_node_list[temp_node_no].outgoing_link_list:
                ...
                for travel_time in range(g_agent_list[agent_no].min_link_travel_time[link], g_agent_list[agent_no].max_link_travel_time[link] + 1):
                    if(m_label_cost[from_node][t] < MAX_LABEL_COST - 1):
                        ...
                        new_to_node_arrival_time = min(
                            t + travel_time, g_number_of_simulation_intervals - 1)
                        ...
                        n_position = g_agent_list[agent_no].set_of_allowed_links.index(
                            link)
                        ...
                        if ((g_link_list[link].link_type != 11) & (g_link_list[link].link_type != 12)):
                            ...
                            if(n_position == 0):  # First section
                                ...
                                # next->arrival event
                                if (g_link_list[next_link_no].link_type == 11):
                            ...
                            # Last section
                            elif(n_position == len(g_agent_list[agent_no].set_of_allowed_links) - 2):
                                ...
                                # former->departure event
                                if(g_link_list[former_link_no].link_type == 11):
                            ...
                            for time in range(TA_left_time, min(TA_left_time + TA_left_time_headway, g_number_of_simulation_intervals)):
                                time_temp = min(
                                    time, g_number_of_simulation_intervals)
                                sum_of_multipliers = sum_of_multipliers + \
                                    g_link_list[link].time_dependent_departure_LR_multiplier_matrix[time_temp]
                            ...
                            for time in range(TD_right_time, min(TD_right_time + TD_right_time_headway, g_number_of_simulation_intervals)):
                                time_temp = min(
                                    time, g_number_of_simulation_intervals)
                                sum_of_multipliers = sum_of_multipliers + \
                                    g_link_list[link].time_dependent_arrival_LR_multiplier_matrix[time_temp]

                        temporary_label_cost = m_label_cost[from_node][t] + \
                            travel_cost + sum_of_multipliers
...
def find_ST_path_for_agents_LR(arg, LR_iteration):
    ...
    return_value, m_label_cost, m_node_predecessor, m_time_predecessor = \
        optimal_ST_dynamic_programming_LR(
            arg, m_earlest_departure_time, m_arrival_time, LR_iteration)
    ...
    # Running time
    travel_time_return_value = path_time_seq[node_size - 1] - path_time_seq[0]
    ...
    for n in range(0, len(p_agent.path_node_id_vector)-1):
        # Waitting at here
        if (node_departure_time == p_agent.path_timestamp_vector[n+1]):
    ...
    for n in range(0, len(p_agent.path_new_node_id_vector)-1):
        p_agent.path_link_TA_vector.append(
            p_agent.path_new_timestamp_vector[temp_number])
        temp_number = temp_number + 2

    temp_number = 3
    for n in range(1, len(p_agent.path_new_node_id_vector)):
        p_agent.path_link_TD_vector.append(
            p_agent.path_new_timestamp_vector[temp_number])
        temp_number = temp_number + 2

    for n in range(0, len(p_agent.path_new_node_id_vector)-1):
        from_node_no = list(g_internal_node_seq_no_dict.keys())[list(
            g_internal_node_seq_no_dict.values()).index(p_agent.path_new_node_id_vector[n])]
        to_node_no = list(g_internal_node_seq_no_dict.keys())[list(
            g_internal_node_seq_no_dict.values()).index(p_agent.path_new_node_id_vector[n+1])]
    ...
    if(LR_iteration == 0):
        p_agent.free_flow_travel_time = p_agent.path_link_TD_vector[n -
                                                                    1] - p_agent.path_link_TD_vector[0]
        p_agent.travel_time_in_dual_solution = p_agent.path_link_TD_vector[n -
                                                                           1] - p_agent.path_link_TD_vector[0]
    else:
        p_agent.travel_time_in_dual_solution = p_agent.path_link_TD_vector[n -
                                                                           1] - p_agent.path_link_TD_vector[0]
                ...
                for travel_time in range(g_agent_list[agent_no].min_link_travel_time[link], g_agent_list[agent_no].max_link_travel_time[link] + 1):
                    if(m_label_cost[from_node][t] < MAX_LABEL_COST - 1):
                        ...
                        if ((g_link_list[link].link_type != 11) & (g_link_list[link].link_type != 12)):
                            ...
                            # Last section
                                ...
                                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                                    ...
                                    TD_right_time_headway = g_agent_list[agent_no].max_link_travel_time[
                                        next_link_no] + g_arrival_headway_stop
                                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                                else:  # passing event
                                    TD_right_time_headway = g_arrival_headway_passing
                                ...
                                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                                    ...
                                    TA_left_time_headway = g_agent_list[agent_no].max_link_travel_time[
                                        former_link_no] + g_departure_headway_stop
                                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                                else:  # passing event
                                    TA_left_time_headway = g_departure_headway_passing
                        ...
                        if ((g_link_list[link].link_type != 11) & (g_link_list[link].link_type != 12)):
                            ...
                            for time in range(TA_left_time, min(TA_left_time + TA_left_time_headway, g_number_of_simulation_intervals)):
                                temp_time = min(
                                    time, g_number_of_simulation_intervals - 1)
                        ...
                        if (isFeasible == 0):
                            ...
                            for time in range(TA_left_time, min(TA_left_time + TA_left_time_headway, g_number_of_simulation_intervals)):
                                temp_time = min(
                                    time, g_number_of_simulation_intervals - 1)
                                sum_of_multipliers = sum_of_multipliers + \
                                    g_link_list[link].time_dependent_departure_LR_multiplier_matrix[temp_time]
                            # TD
                            for time in range(TD_right_time, min(TD_right_time + TD_right_time, g_number_of_simulation_intervals)):
                                temp_time = min(
                                    time, g_number_of_simulation_intervals - 1)
                                sum_of_multipliers = sum_of_multipliers + \
                                    g_link_list[link].time_dependent_arrival_LR_multiplier_matrix[temp_time]
...
def find_ST_path_for_agents(arg, LR_iteration):
    ...
    for temp_i in range(0, node_size):  # Store node_upper_bound into agent
        p_agent.path_node_id_vector_upper_bound.append(list(g_internal_node_seq_no_dict.keys())[
                                                       list(g_internal_node_seq_no_dict.values()).index(path_node_seq[temp_i])])
        p_agent.path_timestamp_vector_upper_bound.append(path_time_seq[temp_i])

    for i in range(0, node_size-1):
        for link in g_link_list:
            if ((path_node_seq[i] == link.from_node_seq_no) & (path_node_seq[i+1] == link.to_node_seq_no)):
                p_agent.path_link_seq_no_vector_upper_bound.append(
                    link.link_seq_no)
                break
    ...
    for n in range(0, len(p_agent.path_node_id_vector_upper_bound)-1):
        # Waitting at here
        if (node_departure_time == p_agent.path_timestamp_vector_upper_bound[n+1]):
            ...
            p_agent.path_new_timestamp_vector_upper_bound.append(
                node_arrival_time)
            p_agent.path_new_timestamp_vector_upper_bound.append(
                node_departure_time)
    ...
    for n in range(0, len(p_agent.path_new_node_id_vector_upper_bound)-1):
        p_agent.path_link_TA_vector_upper_bound.append(
            p_agent.path_new_timestamp_vector_upper_bound[temp_number])
        temp_number = temp_number + 2

    temp_number = 3
    for n in range(1, len(p_agent.path_new_node_id_vector_upper_bound)):
        p_agent.path_link_TD_vector_upper_bound.append(
            p_agent.path_new_timestamp_vector_upper_bound[temp_number])
        temp_number = temp_number + 2
    ...
    for n in range(0, len(p_agent.path_new_node_id_vector_upper_bound)-1):
        ...
        for link in g_link_list:
            if ((from_node_no == link.from_node_id) & (to_node_no == link.to_node_id)):
                temp_link_no = link.link_seq_no
                p_agent.path_new_link_id_vector_upper_bound.append(
                    temp_link_no)
    ...
    for l in range(0, len(p_agent.path_new_link_id_vector_upper_bound)):
        ...
        if ((g_link_list[link_seq_no].link_type != 11) & (g_link_list[link_seq_no].link_type != 12)):
            ...
            if(l == 0):  # First section in this solution
                ...
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    ...
                    TD_right_time_headway = g_agent_list[agent_no].max_link_travel_time[next_link_no] + \
                        g_arrival_headway_stop
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                else:  # passing event
                    TD_right_time_headway = g_arrival_headway_passing
                ...
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    ...
                    TA_left_time_headway = g_agent_list[agent_no].max_link_travel_time[former_link_no] + \
                        g_departure_headway_stop
                # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                else:  # passing event
                    TA_left_time_headway = g_departure_headway_passing
        ...
        if (g_link_list[link_seq_no].link_type == 13):
            TA_left_time_headway = g_agent_list[agent_no].max_link_travel_time[link_seq_no] + \
                g_arrival_headway_stop
            TD_right_time_headway = g_agent_list[agent_no].max_link_travel_time[link_seq_no] + \
                g_departure_headway_stop
        ...
        # Marking other arcs for microscope network
        if (g_link_list[link_seq_no].link_type == 0) and (p_agent.agent_type == 'micro agent'):
            ...
            if ((g_link_list[link_seq_no].link_type != 11) & (g_link_list[link_seq_no].link_type != 12)):
                ...
                if(l == 0):  # First section
                    ...
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                        ...
                        TD_right_time_headway = g_agent_list[agent.agent_id].max_link_travel_time[
                            next_link_no] + g_arrival_headway_stop
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    else:  # passing event
                        TD_right_time_headway = g_arrival_headway_passing
                    ...
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                        ...
                        TA_left_time_headway = g_agent_list[agent.agent_id].max_link_travel_time[
                            former_link_no] + g_departure_headway_stop
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    else:  # passing event
                        TA_left_time_headway = g_departure_headway_passing
...
def g_train_ranking(LR_iteration):
    ...
    for agent in g_agent_list:
        ...
        # # # # # #
        # Choose ranking key as total travel time
        g_agent_DeviationRatio.append(free_trip_time)

    g_agent_sequence = sorted(range(len(g_agent_DeviationRatio)),
                              key=lambda k: g_agent_DeviationRatio[k], reverse=True)
...
def g_upper_bound_solution_feasibility_check():
    ...
    for link in g_link_list:
        link.g_link_time_departure_visit_counts_in_upper_bound = [
            0] * g_number_of_simulation_intervals
        link.g_link_time_arrival_visit_counts_in_upper_bound = [
            0] * g_number_of_simulation_intervals
        ...
        for t in range(0, g_number_of_simulation_intervals):
            ...
            for a in range(0, g_number_of_agents):
                link.g_link_time_departure_train_visit_flag_in_upper_bound[t].append(
                    0)
                link.g_link_time_arrival_train_visit_flag_in_upper_bound[t].append(
                    0)
            ...
            if ((g_link_list[link_seq_no].link_type != 11) & (g_link_list[link_seq_no].link_type != 12)):
                ...
                if(l == 0):  # First section in this solution
                    ...
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                        ...
                        TD_right_time_headway = g_agent_list[p_agent.agent_id].max_link_travel_time[
                            next_link_no] + g_arrival_headway_stop
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    else:  # passing event
                        TD_right_time_headway = g_arrival_headway_passing
                    ...
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                        ...
                        TA_left_time_headway = g_agent_list[p_agent.agent_id].max_link_travel_time[
                            former_link_no] + g_departure_headway_stop
                    # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
                    else:  # passing event
                        TA_left_time_headway = g_departure_headway_passing
    ...
    for agent in g_agent_list:
        for link_no in range(0, len(agent.g_output_link_NO_vector)):
            ...
            if(g_link_list[link].name != ''):
                ...
                # Stopping
                if ((agent.g_output_link_TD_vector[link_no] - agent.g_output_link_TA_vector[link_no]) != 1):
                    station_name = g_link_list[link].name
                    key = (
                        station_name, 'stop', g_link_list[link].from_node_id, g_link_list[link].to_node_id)
                    ...
                    micro_agent.o_node_seq_no = g_workbench_list[
                        workbench_index].g_internal_node_seq_no_dict[micro_agent.o_node_id]
                    micro_agent.d_node_seq_no = g_workbench_list[
                        workbench_index].g_internal_node_seq_no_dict[micro_agent.d_node_id]
                    ...
                    micro_agent.earliest_departure_time = agent.g_output_link_TA_vector[
                        link_no] * 20
                    micro_agent.arrival_time = agent.g_output_link_TD_vector[link_no] * 20

                    micro_agent.set_of_allowed_links = list(
                        map(int, g_agent_map_dict[key].allowed_links_list))
                    micro_agent.min_link_travel_time = list(
                        map(int, g_agent_map_dict[key].min_link_travel_time))
                    micro_agent.max_link_travel_time = list(
                        map(int, g_agent_map_dict[key].max_link_travel_time))
                    micro_agent.headway = list(
                        map(int, g_agent_map_dict[key].headway))

                    micro_agent.agent_type = 'micro agent'
                    g_workbench_list[workbench_index].g_agent_list.append(
                        micro_agent)
                ...
                if ((agent.g_output_link_TD_vector[link_no] - agent.g_output_link_TA_vector[link_no]) == 1):
                    station_name = g_link_list[link].name
                    key = (
                        station_name, 'over', g_link_list[link].from_node_id, g_link_list[link].to_node_id)
...
def g_UpdateData_parameter(workbench):
    if (workbench.name == 'macro network'):
        workbench.MAX_NUMBER_OF_TIME_INTERVALS = 60
        ...
        workbench.MAX_NUMBER_OF_TIME_INTERVALS = 60 * 20
        workbench.g_number_of_simulation_intervals = workbench.MAX_NUMBER_OF_TIME_INTERVALS
    ...
    with open('map.csv', 'r') as fp:
        ...
        for l in lines[1:]:
            ...
            min_link_travel_time_list = l[min_link_travel_time].strip().split(
                ';')
            agent.min_link_travel_time = [
                max(1, math.floor(int(i)/3)) for i in min_link_travel_time_list]
            max_link_travel_time_list = l[max_link_travel_time].strip().split(
                ';')
            agent.max_link_travel_time = [
                math.ceil(int(i)/3) for i in max_link_travel_time_list]

            agent.headway = l[headway].strip().split(';')
            g_agent_map.append(agent)
            key = (agent.station_name, agent.type_name,
                   agent.macro_from_node_id, agent.macro_to_node_id)
...
def g_optimization(workbench):
    ...
    for link in g_link_list:
        for t in range(0, g_number_of_simulation_intervals):
            link.time_dependent_departure_LR_multiplier_matrix.append(
                g_initial_LR_multiplier_value)
            link.time_dependent_arrival_LR_multiplier_matrix.append(
                g_initial_LR_multiplier_value)
    ...
    while (LR_iteration <= g_number_of_LR_iterations) or (is_upperbound_feasible == 0):
        ...
        for link in g_link_list:
            ...
            for t in range(0, g_number_of_simulation_intervals):
                ...
                if(link.same_link_id != 1000):
                    ...
                    link_visit_counts = link_visit_counts + \
                        g_link_list[same_link_no].g_link_time_departure_visit_counts[t]
                ...
                if(link.same_link_id != 1000):
                    same_link_no = g_internal_link_seq_no_dict[link.same_link_id]
                    link_visit_counts = link_visit_counts + \
                        g_link_list[same_link_no].g_link_time_arrival_visit_counts[t]
        ...
        for link in g_link_list:
            for t in range(0, g_number_of_simulation_intervals):
                if (link.link_type == 11 or link.link_type == 12):
                    link.time_depedent_capacity_matrix.append(
                        4)  # capacity = 4 in side track
                else:
        ...
        if (workbench.name == 'macro network'):
            ...
            for link in g_link_list:
                for t in range(0, g_number_of_simulation_intervals):
                    total_resource_price = total_resource_price + \
                        link.time_dependent_arrival_LR_multiplier_matrix[t]
                    total_resource_price = total_resource_price + \
                        link.time_dependent_departure_LR_multiplier_matrix[t]
        ...
        for agent in g_agent_list:
            if (len(agent.path_new_link_id_vector_upper_bound) == 0):
                ...
                is_upperbound_feasible = 0
            # the last one is dummy link
            dummy_link_id = agent.set_of_allowed_links[-1]
            dummy_link_no = g_internal_link_seq_no_dict[dummy_link_id]
            if dummy_link_no in agent.path_new_link_id_vector_upper_bound:
                effective_agent[agent.agent_id] = 0
                is_upperbound_feasible = 0
                print('Not path found in agent ' +
                      str(agent.agent_id) + '(using dunmmy links)')
                # break
        ...
        if(is_upperbound_feasible == 1):
            for agent in g_agent_list:
                ...
                total_travel_time += (actual_arrival_time -
            ...
            if (workbench.name == 'macro network') and (LR_iteration == g_number_of_LR_iterations):
                ...
                print("Gap = " + str((g_best_upper_bound -
                      g_best_lower_bound) * 100 / g_best_upper_bound) + "%")
            if (workbench.name != 'macro network'):
    ...
    for agent in g_agent_list:
        if (effective_agent[agent.agent_id] == 1):
            ...
            for j in range(0, len(agent.g_output_link_NO_vector)):
                ...
                x_temp_from = g_link_list[agent.g_output_link_NO_vector[j]
                                          ].from_node_seq_no
                x_temp_to = g_link_list[agent.g_output_link_NO_vector[j]
                                        ].to_node_seq_no
        ...
        if (link.name == 'W' + str(station_no)):
            ...
            for agent in macro_workbench.g_agent_list:
                if (link_id in agent.g_output_link_NO_vector):
                    agent_link_index = agent.g_output_link_NO_vector.index(
                        link_id)
                    if(effective_agent[agent.agent_id] == 0) and (agent.direction == macro_workbench.g_link_list[link_id].direction):
                        ...
                        for temp_agent in macro_workbench.g_agent_list:
                            ...
                            if (agent.direction == temp_agent.direction):
                                try:  # Skip out the stop-pass-stop headway
                                    same_former_agent_link_index = \
                                        same_former_agent.g_output_link_NO_vector.index(
                                            link_id)
                                    temp_agent_link_index = temp_agent.g_output_link_NO_vector.index(
                                        link_id)
                                    ...
                                    if(temp_agent.g_output_link_TA_vector[temp_agent_link_index] <
                                        ...
                                        same_former_agent_TA = temp_agent.g_output_link_TA_vector[
                                            temp_agent_link_index]
                                        same_flag = 1
                                except:
                            ...
                            if (link.name in ['W7']):  # Only for bi-direction station
                                if (agent.direction != temp_agent.direction):
                                    oppo_link_id = macro_workbench.g_link_list[link_id].oppo_link_id
                                    oppo_former_agent_link_index = \
                                        oppo_former_agent.g_output_link_NO_vector.index(
                                            oppo_link_id)
                                    temp_agent_link_index = temp_agent.g_output_link_NO_vector.index(
                                        oppo_link_id)
                                    # Find the latest former agent
                                    if(temp_agent.g_output_link_TA_vector[temp_agent_link_index] <
                                        ...
                                        oppo_former_agent_TA = temp_agent.g_output_link_TA_vector[
                                            temp_agent_link_index]
                                        oppo_flag = 1
                ...
                for link_no in range(0, len(agent.g_output_link_NO_vector)):
                    ...
                    if (workbench.g_node_list[from_node_seq_no].name != ''):
                        line_1 = [
                            ...
                            # 3s/unit -->1s/unit
                            agent.g_output_link_TA_vector[link_no] * 3
                        ]
...
if __name__ == '__main__':
    ...
    for iter in range(0, g_outer_iteration):
        ...
        for station in g_station_list:
            ...
            g_reset_global_parameter()
            g_UpdateData_parameter(
                g_workbench_list[g_station_list.index(station) + 1])
            ...
            if (is_upperbound_feasible == 0):
                AdjustFeedback(
                    station, g_workbench_list[0], g_workbench_list[g_station_list.index(station) + 1])

        # Obtain Upper bound and Lower Bound for each inner iteration
        g_lower_bound_temp = g_workbench_list[0].g_best_lower_bound * 60
        g_upper_bound_temp = g_workbench_list[0].g_best_upper_bound * 60
        g_unfeasible_temp = 0
        ...
        for workbench_no in range(1, len(g_workbench_list)):
            ...
            g_unfeasible_temp = g_unfeasible_temp + \
                workbench.effective_agent.count(0)
            g_conflict_summary[iter][workbench_no -
                                     1] = workbench.effective_agent.count(0)
